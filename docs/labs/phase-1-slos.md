...
| **P1-ISSUE-16** | **Telemetry** | Frontend span metrics stopped **11:17:28–11:22:36**; SLIs and dashboards went **blank**; Locust saw some 503s | **Open**: no pod restarts, 0 exporter send failures, normal CPU afterwards. Hypothesis: the frontend stalled briefly (Envoy 503s) | **Lesson:** when telemetry stops, SLIs go blank rather than red, and alerts can't fire. **Phase 2:** collector self-monitoring + proxy metrics to find the cause. **Phase 3:** add `absent(slo:sli_error:ratio_rate5m)` "SLI data missing" alert | Data resumed on its own at 11:22:36 |
