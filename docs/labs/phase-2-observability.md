...
| P2-ISSUE-22 | Loki | After the long sleep: collectors got **HTTP 503 "at least 1 live replicas required, could only find 0"** from Loki; logs were being dropped | During the VM freeze Loki missed its own ring heartbeat and **evicted itself from its ring** (`auto-forgetting instance from the ring because it is unhealthy`) | **Recurred** after the next host sleep (03:05–03:11 UTC). A restart re-registers it, but a repeat means config: ring `heartbeat_timeout` 1m → **10m** (ingester, distributor, compactor) via `loki.structuredConfig`, so short VM pauses don't evict the single instance. Verified in the live ConfigMap; 0 503s after. Production answer: ≥3 replicas |
...
- [~] P2-ISSUE-10 OTLP 400s: two-writer cause fixed (`host.name`); a residual ~1.5% source is **open** (tracked by alert)
